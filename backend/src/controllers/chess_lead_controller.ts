...
import ChessLeaderboard from "../models/chess_lead_model";
...
export const getLeaderboardStandings = async (req: Request, res: Response) => {
  try {
    ...
    // Only completed events count for tournament standings
    const matches = await ChessEvent.find({ 
      event_status: "completed",
      gender: matchGender 
    });
    const leaderboardEntries = await ChessLeaderboard.find(leadQuery);
...
export const createLeaderboardEntry = async (req: Request, res: Response) => {
  try {
    ...
    const entry = new ChessLeaderboard({ leaderboard_id, dept_name, category, group });
    await entry.save();
...
export const getAllLeaderboardEntries = async (req: Request, res: Response) => {
  try {
    const entries = await ChessLeaderboard.find();
...
export const getLeaderboardEntryById = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const entry = await ChessLeaderboard.findOne({ leaderboard_id: Number(id) });
    if (!entry) {
...
export const updateLeaderboardEntry = async (req: Request, res: Response) => {
  try {
    ...
    const updated = await ChessLeaderboard.findOneAndUpdate(
...
export const deleteLeaderboardEntry = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const deleted = await ChessLeaderboard.findOneAndDelete({ leaderboard_id: Number(id) });
    if (!deleted) {
      res.status(404).json({ message: "Entry not found." });
      return;
    }
