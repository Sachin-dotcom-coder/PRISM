...
import ArmWrestlingLeaderboard from "../models/armwrestling_lead_model";
import { Request, Response } from "express";

export const getLeaderboardStandings = async (req: Request, res: Response) => {
  try {
    // Only completed matches
    const matches = await ArmWrestlingEvent.find({ event_status: "completed" });
    // Get group info for each team from leaderboard
    const leaderboardEntries = await ArmWrestlingLeaderboard.find();
...
export const createLeaderboardEntry = async (req: Request, res: Response) => {
  try {
    ...
    const entry = new ArmWrestlingLeaderboard({ leaderboard_id, dept_name, category, group });
    await entry.save();
...
export const getAllLeaderboardEntries = async (req: Request, res: Response) => {
  try {
    const entries = await ArmWrestlingLeaderboard.find();
...
export const getLeaderboardEntryById = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const entry = await ArmWrestlingLeaderboard.findOne({ leaderboard_id: Number(id) });
    if (!entry) {
...
export const updateLeaderboardEntry = async (req: Request, res: Response) => {
  try {
    ...
    const updated = await ArmWrestlingLeaderboard.findOneAndUpdate(
...
export const deleteLeaderboardEntry = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const deleted = await ArmWrestlingLeaderboard.findOneAndDelete({ leaderboard_id: Number(id) });
    if (!deleted) {
      res.status(404).json({ message: "Entry not found." });
      return;
    }
