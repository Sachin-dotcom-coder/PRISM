...
import ChessEvent from "../models/chess_model";

type ChessGender = "men" | "women";

const isValidGender = (value: unknown): value is ChessGender =>
...
export const createChessEvent = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const savedEvent = await ChessEvent.create(validation.payload!);
    res.status(201).json({ success: true, message: "Chess event created.", data: savedEvent });
  } catch (error: any) {
    if (error.code === 11000) {
...
export const getAllChessEvents = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const events = await ChessEvent.find({ gender }).sort({ event_id: 1 });
...
export const getChessEventById = async (req: Request, res: Response): Promise<void> => {
  try {
    const gender = getGenderFromRequest(req);
    if (!gender) {
      res.status(400).json({ success: false, message: "gender query parameter is required." });
    ...
    const event = await ChessEvent.findOne({ event_id: Number(event_id), gender });
    if (!event) {
...
export const updateChessEvent = async (req: Request, res: Response): Promise<void> => {
  try {
    const gender = getGenderFromRequest(req);
    if (!gender) {
    ...
    const { event_id } = req.params;
    const existingEvent = await ChessEvent.findOne({ event_id: Number(event_id), gender });
    if (!existingEvent) {
      res.status(404).json({ success: false, message: "Event not found." });
      return;
    }
    ...
    const updatedEvent = await ChessEvent.findOneAndUpdate(
      { event_id: Number(event_id), gender },
      validation.payload!,
      { new: true, runValidators: true }
    );

    res.status(200).json({ success: true, message: "Chess event updated.", data: updatedEvent });
  } catch (error: any) {
    res.status(500).json({ success: false, message: "Server Error", data: error.message });
  }
};

export const deleteChessEvent = async (req: Request, res: Response): Promise<void> => {
  try {
    const gender = getGenderFromRequest(req);
    if (!gender) {
      res.status(400).json({ success: false, message: "gender query parameter is required." });
    ...
    const { event_id } = req.params;
    const deletedEvent = await ChessEvent.findOneAndDelete({
      event_id: Number(event_id),
      gender
    });
