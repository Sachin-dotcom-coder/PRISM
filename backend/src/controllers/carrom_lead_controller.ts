...
import CarromLeaderboard from "../models/carrom_lead_model";
...
export const createLeaderboardEntry = async (req: Request, res: Response) => {
  try {
    ...
    const entry = new CarromLeaderboard({ leaderboard_id, dept_name, category, group });
    await entry.save();
...
export const getAllLeaderboardEntries = async (req: Request, res: Response) => {
  try {
    const entries = await CarromLeaderboard.find();
...
export const getLeaderboardEntryById = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const entry = await CarromLeaderboard.findOne({ leaderboard_id: Number(id) });
    if (!entry) {
...
export const updateLeaderboardEntry = async (req: Request, res: Response) => {
  try {
    ...
    const updated = await CarromLeaderboard.findOneAndUpdate(
...
export const deleteLeaderboardEntry = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const deleted = await CarromLeaderboard.findOneAndDelete({ leaderboard_id: Number(id) });
    if (!deleted) {
      res.status(404).json({ message: "Entry not found." });
      return;
    }
