...
export default function Navbar() {
  ...
  return (
    <>
      ...
      {/* Changed 'fixed' to 'absolute' here so it scrolls away */}
      <nav className={`absolute top-0 left-0 right-0 z-50 glass border-b h-24 flex items-center px-6 md:px-12 justify-between ${gender === 'f' ? 'border-zinc-200 bg-white/50 text-zinc-900' : 'border-white/5'}`}>
        ...
        <div className="hidden md:flex items-center gap-10">
          ...
          {/* Updated link: now points to the /committee page */}
          <Link href="/committee" className="text-sm font-bold tracking-[0.2em] uppercase text-gray-100 hover:text-[#FFBF00] transition-colors drop-shadow-none">Core Committee</Link>
