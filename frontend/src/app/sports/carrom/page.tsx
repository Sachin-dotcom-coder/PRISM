...
// Make sure this points to your backend port (5000) based on your earlier setup
...
function ScorecardContent({ match, isLive = false }: { match: any, isLive?: boolean }) {
  ...
  const teamA = match.teams?.player_a || match.teams?.team_a || { name: "Player A", score: 0, boards_won: 0 };
  const teamB = match.teams?.player_b || match.teams?.team_b || { name: "Player B", score: 0, boards_won: 0 };
  const currentBoard = match.current_board || 1;
  const result = match.result || "Ongoing";

  // Calculate Lead based on points
  const scoreDiff = teamA.score - teamB.score;
  let leadText = "Scores Level";
  if (scoreDiff > 0) leadText = `${teamA.name} leads by ${scoreDiff} PTS`;
  if (scoreDiff < 0) leadText = `${teamB.name} leads by ${Math.abs(scoreDiff)} PTS`;
      ...
      <div className="border border-[#1A1A1A] bg-[#000000] rounded-2xl p-5 md:p-8 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center w-full gap-8 md:gap-0 z-10 relative">
          ...
          <div className="flex flex-col items-center md:items-start w-full md:w-2/5">
            ...
            <div className="mt-2 bg-[#111] border border-[#333] px-3 py-1 rounded text-xs font-mono tracking-widest text-gray-400">
              BOARDS WON: <span className="text-[#FFBF00] font-bold text-sm ml-1">{teamA.boards_won || 0}</span>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center md:items-end w-full md:w-2/5">
            ...
            <div className="mt-2 bg-[#111] border border-[#333] px-3 py-1 rounded text-xs font-mono tracking-widest text-gray-400">
              BOARDS WON: <span className="text-[#FFBF00] font-bold text-sm ml-1">{teamB.boards_won || 0}</span>
            </div>
          </div>
        </div>
      </div>
            ...
            <table className="w-full text-left text-base whitespace-nowrap bg-transparent rounded-lg overflow-hidden">
              <thead>
                <tr className="text-gray-400 border-b border-[#333] uppercase text-sm md:text-base font-bold tracking-widest">
                  ...
                  <th className="py-4 px-4">Player/Team</th>
