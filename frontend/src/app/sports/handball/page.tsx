...
import { fetchHandballMatches, fetchHandballStandings, HandballMatch, GroupedStandings } from "../../../lib/sportsAPI";
    ...
    <div className="min-h-screen bg-[#000000] text-[#FFFFFF] font-sans flex justify-center selection:bg-[#FFBF00] selection:text-black">
      <div className="w-full max-w-7xl p-4 sm:p-8">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 border-b border-[#1A1A1A] pb-6 gap-4">
          ...
          <p className="text-sm text-gray-400 font-medium tracking-wide"><span className="text-gray-200">Event:</span> PRISM 2026 <span className="mx-2 text-[#333]">|</span> <span className="text-gray-200">Venue:</span> SVNIT Main Ground</p>
        </header>
...
function Results({ gender }: { gender: "men" | "women" }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const all = await fetchHandballMatches(gender);
        ...
        <div key={match._id} className="border border-[#1A1A1A] bg-[#000000] rounded-2xl p-5 md:p-6 hover:border-[#FFBF00]/20 transition-colors">
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 md:gap-6">
              <span className={`text-5xl md:text-6xl font-black tracking-tighter ${match.winner === match.team1_department ? "text-[#FFBF00]" : "text-white"}`}>{match.team1_score ?? "—"}</span>
              <span className="text-2xl font-black text-[#333]">—</span>
              <span className={`text-5xl md:text-6xl font-black tracking-tighter ${match.winner === match.team2_department ? "text-[#FFBF00]" : "text-white"}`}>{match.team2_score ?? "—"}</span>
            </div>
...
function Leaderboard({ gender }: { gender: "men" | "women" }) {
  ...
  const load = useCallback(async () => {
    setLoading(true); setError("");
    try { setGrouped(await fetchHandballStandings(category)); }
