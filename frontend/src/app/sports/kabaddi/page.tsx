    ...
    <div className="min-h-screen bg-[#000000] text-[#FFFFFF] font-sans flex justify-center selection:bg-[#FFBF00] selection:text-black">
      ...
      <div className="w-full max-w-7xl p-4 sm:p-8">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 border-b border-[#1A1A1A] pb-6 gap-4">
          <h1 className="text-[#FFBF00] font-black text-6xl md:text-7xl tracking-tighter uppercase">Kabaddi</h1>
          <div className="text-left md:text-right text-sm md:text-base text-gray-400 space-y-1 font-medium tracking-wide">
            ...
            <p><span className="text-gray-200">Venue:</span> SVNIT Kabaddi Ground</p>
          </div>
        </header>
...
function ScorecardContent({ match, isLive = false }: { match: any, isLive?: boolean }) {
  ...
  const teamA = match.teams?.team_a || { name: "Team A", score: 0 };
  const teamB = match.teams?.team_b || { name: "Team B", score: 0 };
  const halves = match.halves || [];
  const recentRaids = match.recent_raids || [];
  const currentHalf = match.current_half || 1;

  // Calculate Lead
  const scoreDiff = teamA.score - teamB.score;
  let leadText = "Scores Level";
  if (scoreDiff > 0) leadText = `${teamA.name} leads by ${scoreDiff}`;
  if (scoreDiff < 0) leadText = `${teamB.name} leads by ${Math.abs(scoreDiff)}`;
  ...
  return (
    <div className="space-y-8">
      ...
      {isLive && (
        <div className="bg-[#000000] border border-[#FFBF00]/30 rounded-xl p-4 sm:p-5 flex justify-center items-center gap-6 shadow-[0_0_15px_rgba(255,191,0,0.05)] mx-auto w-fit">
          <div className="flex flex-col justify-center items-center leading-none font-bold text-xs tracking-widest uppercase">
            <span className="text-gray-400 mb-2">Current Status</span>
            <span className="text-xl sm:text-2xl font-black text-[#FFBF00] tracking-tighter">
              {leadText}
            </span>
          </div>
        </div>
      ...
      {halves.length > 0 && (
        <div className="mt-8">
          <h3 className="text-[#FFBF00] font-black text-xl mb-4 tracking-widest uppercase pl-1">Halves Breakdown</h3>
              ...
              <tbody className="divide-y divide-[#1A1A1A]">
                {halves.map((half: any, i: number) => (
                  <tr key={i} className="hover:bg-[#0A0A0A] transition-colors">
                    ...
                    <td className="py-4 px-4 text-center font-black text-gray-300 text-xl">
                      {half.team_a_score}
                    </td>
                    <td className="py-4 px-4 text-center font-black text-gray-300 text-xl">
                      {half.team_b_score}
      ...
      {recentRaids.length > 0 && (
        <div className="mt-10">
          <h3 className="text-gray-300 font-black text-xl mb-4 tracking-widest uppercase pl-1 flex items-center gap-3">
            Recent Raids
            {isLive && <span className="w-2 h-2 bg-red-500 rounded-full animate-ping"></span>}
          </h3>
          <div className="overflow-x-auto rounded-xl border border-[#1A1A1A] p-3">
            <table className="w-full text-left text-base md:text-lg whitespace-nowrap">
              <thead>
                <tr className="text-gray-500 border-b border-[#1A1A1A] uppercase tracking-wider text-sm font-bold">
                  <th className="py-4 px-4">Raider</th>
                  <th className="py-4 px-4">Result</th>
                  <th className="py-4 px-4 text-right text-[#FFBF00]">Points Scored</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#1A1A1A]">
                {recentRaids.map((raid: any, i: number) => {
                  const isSuccess = raid.result === "SUCCESSFUL";
                  return (
                    <tr key={raid.raid_id || i} className="hover:bg-[#0A0A0A] transition-colors group">
                      <td className="py-4 px-4 font-medium text-white tracking-wide">
                        {raid.raider}
                      </td>
                      <td className="py-4 px-4">
                        <span className={`text-[10px] md:text-xs font-black px-3 py-1 rounded-sm uppercase tracking-widest ${
                          isSuccess ? "bg-[#FFBF00]/10 text-[#FFBF00] border border-[#FFBF00]/20" : "bg-red-500/10 text-red-500 border border-red-500/20"
                        }`}>
                          {raid.result}
                        </span>
                      </td>
                      <td className={`py-4 px-4 text-right font-black text-xl md:text-2xl ${isSuccess ? "text-white" : "text-gray-600"}`}>
                        {isSuccess ? `+${raid.points_scored}` : "0"}
                      </td>
                    </tr>
                  );
                })}
            ...
            <table className="w-full text-left text-base whitespace-nowrap bg-transparent rounded-lg overflow-hidden">
              <thead>
                <tr className="text-gray-400 border-b border-[#333] uppercase text-sm md:text-base font-bold tracking-widest">
                  ...
                  <th className="py-4 px-4 text-right">Score Diff</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#1A1A1A]">
                {groups[groupName]
                  .sort((a: any, b: any) => b.points - a.points || (b.scoreDiff || 0) - (a.scoreDiff || 0))
                  ...
                  <tr key={team._id || index} className="hover:bg-[#0A0A0A] transition-colors">
                    ...
                    <td className="py-5 px-3 text-center text-gray-500">{team.ties || 0}</td>
                    <td className="py-5 px-3 text-center font-black text-xl text-white">{team.points || 0}</td>
                    <td className="py-5 px-4 text-right text-gray-400 font-mono">{(team.scoreDiff > 0 ? "+" : "")}{team.scoreDiff || 0}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="mb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {list.map((match: any, index: number) => {
            const teamAObj = match.teams?.team_a;
            const teamBObj = match.teams?.team_b;
