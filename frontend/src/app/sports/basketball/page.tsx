    ...
    <div className="min-h-screen bg-[#000000] text-[#FFFFFF] font-sans flex justify-center selection:bg-[#FFBF00] selection:text-black">
      <div className="w-full max-w-7xl p-4 sm:p-8">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 border-b border-[#1A1A1A] pb-6 gap-4">
          ...
          <p className="text-sm text-gray-400 font-medium tracking-wide"><span className="text-gray-200">Event:</span> PRISM 2026 <span className="mx-2 text-[#333]">|</span> <span className="text-gray-200">Venue:</span> SVNIT Indoor Court</p>
        </header>
...
function Results({ gender }: { gender: "men" | "women" }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const all = await fetchBasketballMatches(gender);
  ...
  return (
    <div className="space-y-6">
      ...
      {matches.map((match) => {
        const s1 = match.games.reduce((acc, g) => acc + (g.team1_score ?? 0), 0);
        const s2 = match.games.reduce((acc, g) => acc + (g.team2_score ?? 0), 0);
        return (
          <div key={match._id} className="border border-[#1A1A1A] bg-[#000000] rounded-2xl p-5 md:p-6 hover:border-[#FFBF00]/20 transition-colors">
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3 md:gap-5">
                <span className={`text-4xl md:text-5xl font-black tracking-tighter ${match.winner === match.team1_department ? "text-[#FFBF00]" : "text-white"}`}>{s1}</span>
                <span className="text-2xl font-black text-[#333]">—</span>
                <span className={`text-4xl md:text-5xl font-black tracking-tighter ${match.winner === match.team2_department ? "text-[#FFBF00]" : "text-white"}`}>{s2}</span>
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-[#1A1A1A] flex flex-wrap gap-2 justify-center">
                {match.games.map((g, i) => (
                  <div key={i} className="flex flex-col items-center bg-[#0A0A0A] border border-[#1A1A1A] rounded-lg px-3 py-2 min-w-[56px]">
                    <span className="text-gray-500 text-[10px] font-bold uppercase tracking-widest mb-1">Q{g.game_number}</span>
                    <span className="text-white text-sm font-black">{g.team1_score}–{g.team2_score}</span>
                  </div>
                ))}
              </div>
            )}
...
function Leaderboard({ gender }: { gender: "men" | "women" }) {
  ...
  const load = useCallback(async () => {
    setLoading(true); setError("");
    try { setGrouped(await fetchBasketballStandings(category)); }
