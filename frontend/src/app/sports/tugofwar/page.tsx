...
import {
  fetchTugOfWarMatches,
  fetchTugOfWarStandings,
  TugOfWarMatch,
  GroupedStandings,
} from "../../../lib/sportsAPI";
    ...
    <div className="min-h-screen bg-[#000000] text-[#FFFFFF] font-sans flex justify-center selection:bg-[#FFBF00] selection:text-black pt-20">
      <div className="w-full max-w-7xl p-4 sm:p-8">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 border-b border-[#1A1A1A] pb-6 gap-4">
          <h1 className="text-[#FFBF00] font-black text-6xl md:text-7xl tracking-tighter uppercase">Tug of War</h1>
          <p className="text-sm text-gray-400 font-medium tracking-wide"><span className="text-gray-200">Event:</span> PRISM 2026 <span className="mx-2 text-[#333]">|</span> <span className="text-gray-200">Venue:</span> Main Ground</p>
        </header>
...
function Results({ gender }: { gender: "men" | "women" }) {
  const [matches, setMatches] = useState<TugOfWarMatch[]>([]);
  ...
  const load = useCallback(async (isInitial = false) => {
    ...
    try {
      const all = await fetchTugOfWarMatches(gender);
      setMatches(all.filter((m) => m.event_status === "completed"));
    } catch (err) { console.error(err); setError("Failed to load event results."); }
  ...
  if (!matches.length) return <EmptyState label="No completed pulls yet." />;

  return (
    <div className="space-y-6">
      <h2 className="text-white font-black text-2xl tracking-widest uppercase mb-6">Match Results</h2>
      {matches.map((match) => (
        <div key={match._id} className="border border-[#1A1A1A] bg-[#000000] rounded-2xl p-6 hover:border-[#FFBF00]/20 transition-colors relative overflow-hidden group">
          <div className="flex justify-between items-center mb-6">
            <span className="text-[#FFBF00] text-xs font-bold uppercase tracking-widest">#{match.event_id}</span>
          ...
          <div className="flex items-center justify-between gap-4">
            <div className={`flex flex-col flex-1 text-center md:text-left ${match.winner === match.department_1 ? "text-[#FFBF00]" : "text-white"}`}>
              <span className="font-black text-2xl md:text-4xl tracking-widest uppercase">{match.department_1}</span>
              {match.winner === match.department_1 && <span className="text-xs font-bold tracking-widest mt-2 text-[#FFBF00]">WINNER</span>}
            </div>
            <div className="flex flex-col items-center gap-1 group-hover:scale-110 transition-transform">
              <span className="text-3xl font-black text-[#222]">VS</span>
            </div>
            <div className={`flex flex-col flex-1 text-center md:text-right ${match.winner === match.department_2 ? "text-[#FFBF00]" : "text-white"}`}>
              <span className="font-black text-2xl md:text-4xl tracking-widest uppercase">{match.department_2}</span>
              {match.winner === match.department_2 && <span className="text-xs font-bold tracking-widest mt-2 text-[#FFBF00]">WINNER</span>}
            </div>
          </div>
          <div className="mt-6 pt-4 border-t border-[#1A1A1A] flex justify-between items-center text-[10px] font-mono text-zinc-500 uppercase tracking-widest">
            <span>{fmtDate(match.event_date)}</span>
            <span>{match.venue || "SVNIT Main Ground"}</span>
          </div>
        </div>
      ))}
    </div>
...
function Leaderboard({ gender }: { gender: "men" | "women" }) {
  ...
  const load = useCallback(async (isInitial = false) => {
    if (isInitial) setLoading(true); setError("");
    try { 
      const res = await fetchTugOfWarStandings(gender);
    ...
    <div className="space-y-12">
      {groups.map(([grp, teams]) => (
        <div key={grp} className="bg-[#000000] p-6 rounded-xl border border-[#1A1A1A]">
          ...
          <p className="text-gray-600 text-[10px] mt-4 tracking-widest uppercase font-bold">Win = 3 pts · Pull Win = 3 pts</p>
        </div>
      ))}
    </div>
  );
...
function Fixtures({ gender }: { gender: "men" | "women" }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {matches.map((m) => (
        <div key={m._id} className="bg-[#000000] border border-[#1A1A1A] border-l-8 border-l-[#FFBF00] p-6 rounded-r-xl transition-all hover:scale-[1.03] group shadow-[0_0_20px_rgba(255,191,0,0.05)]">
          <div className="flex justify-between items-start mb-6">
            ...
            <div className="flex flex-col items-end gap-1">
              <span className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">#{m.event_id}</span>
              {m.event_status === "ongoing" && <span className="text-[10px] font-black uppercase tracking-widest text-[#FFBF00] animate-pulse">● LIVE PULL</span>}
            </div>
          </div>
          <h4 className="text-2xl font-black text-white mb-0 uppercase group-hover:text-[#FFBF00] transition-colors leading-tight">
            {m.department_1}
          </h4>
          <div className="text-xs font-black text-zinc-800 my-1 font-mono">VERSUS</div>
          <h4 className="text-2xl font-black text-white mb-4 uppercase group-hover:text-[#FFBF00] transition-colors leading-tight">
            {m.department_2}
          </h4>
          <div className="pt-4 border-t border-[#1A1A1A] text-xs text-gray-500 font-medium tracking-wide">
            {fmtDate(m.event_date)} • {m.venue || "SVNIT Main Ground"}
          </div>
        </div>
      ))}
    </div>
