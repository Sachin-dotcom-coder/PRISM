...
import { fetchKhoKhoMatches, fetchKhoKhoStandings, KhoKhoMatch, GroupedStandings } from "../../../lib/sportsAPI";
    ...
    <div className="min-h-screen bg-[#000000] text-[#FFFFFF] font-sans flex justify-center selection:bg-[#FFBF00] selection:text-black">
      <div className="w-full max-w-7xl p-4 sm:p-8">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 border-b border-[#1A1A1A] pb-6 gap-4">
          ...
          <p className="text-sm text-gray-400 font-medium tracking-wide"><span className="text-gray-200">Event:</span> PRISM 2026 <span className="mx-2 text-[#333]">|</span> <span className="text-gray-200">Venue:</span> SVNIT Ground</p>
        </header>
...
function Results({ gender }: { gender: "men" | "women" }) {
  const [matches, setMatches] = useState<KhoKhoMatch[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const all = await fetchKhoKhoMatches(gender);
...
function Leaderboard({ gender }: { gender: "men" | "women" }) {
  ...
  const load = useCallback(async () => {
    setLoading(true); setError("");
    try { setGrouped(await fetchKhoKhoStandings(category)); }
