...
import { getMatches, deleteMatch } from './services/khokhoApi';
import { useGender } from '@/app/components/Providers';

// ─── Types ────────────────────────────────────────────────────────────────────
interface KhoKhoLeaderboardEntry {
  _id?: string;
...
const LB_API = `${BASE_URL}/khokho-leaderboard`;
...
// ─── Leaderboard Row ──────────────────────────────────────────────────────────
function KhoKhoTeamRow({
  team, rank, onUpdate, onDelete,
}: {
  team: KhoKhoLeaderboardEntry;
  rank: number;
  onUpdate: (t: KhoKhoLeaderboardEntry) => void;
  onDelete: () => void;
}) {
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function KhoKhoAdminPage() {
  ...
  // ── Match state ──
  const [matches, setMatches] = useState<IKhoKhoMatch[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [showForm, setShowForm] = useState(false);
  const [editingMatch, setEditingMatch] = useState<IKhoKhoMatch | null>(null);

  // ── Leaderboard state ──
  const [lbEntries, setLbEntries] = useState<KhoKhoLeaderboardEntry[]>([]);
  const [standings, setStandings] = useState<Record<string, any[]>>({});
  const [addMode, setAddMode] = useState(false);
  const [lbMsg, setLbMsg] = useState('');
  const [newEntry, setNewEntry] = useState<Partial<KhoKhoLeaderboardEntry>>({
    dept_name: '',
    category: lbCategory,
    group: 'A',
  });
  ...
  const handleEdit = (match: IKhoKhoMatch) => { setEditingMatch(match); setShowForm(true); };
  ...
  const handleUpdateEntry = async (t: KhoKhoLeaderboardEntry) => {
    // backend uses leaderboard_id (numeric) as :id
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-[900] tracking-widest text-[#FFBF00] uppercase">Kho-Kho CMS</h1>
          </div>
        </div>
            ...
            {filteredEntries.length === 0 ? (
              ...
              <div className="divide-y divide-zinc-800/50">
                {groups.map(gp => {
                  ...
                  const entryLookup: Record<string, KhoKhoLeaderboardEntry> = {};
                  ...
                  const gpTeams: KhoKhoLeaderboardEntry[] = filteredEntries
                    .filter(e => e.group === gp)
                    .map(e => {
