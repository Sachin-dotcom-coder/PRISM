...
import { createEvent, deleteEvent, getEvent, getEvents, updateEvent } from "./services/carromApi";
import { CarromGender, ICarromEvent } from "./types";

// ─── Types ────────────────────────────────────────────────────────────────────
interface CarromLeaderboardEntry {
  _id?: string;
...
const LB_API = `${BASE_URL}/carrom-leaderboard`;
...
// ─── Leaderboard Row ──────────────────────────────────────────────────────────
function CarromTeamRow({
  team, rank, onUpdate, onDelete,
}: {
  team: CarromLeaderboardEntry;
  rank: number;
  onUpdate: (t: CarromLeaderboardEntry) => void;
  onDelete: () => void;
}) {
...
const buildInitialForm = (gender: CarromGender): ICarromEvent => ({
...
export default function CarromAdminPage() {
  const { gender: globalGender } = useGender();
  const gender: CarromGender = globalGender === "f" ? "women" : "men";
  const lbCategory: 'boys' | 'girls' = gender === 'men' ? 'boys' : 'girls';

  const [events, setEvents] = useState<ICarromEvent[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [editingEventId, setEditingEventId] = useState<number | null>(null);
  const [formData, setFormData] = useState<ICarromEvent>(() => buildInitialForm(gender));

  // ── Leaderboard state ──
  const [lbEntries, setLbEntries] = useState<CarromLeaderboardEntry[]>([]);
  const [standings, setStandings] = useState<Record<string, any[]>>({});
  const [addMode, setAddMode] = useState(false);
  const [lbMsg, setLbMsg] = useState("");
  const [newEntry, setNewEntry] = useState<Partial<CarromLeaderboardEntry>>({
  ...
  const fetchEvents = useCallback(async (activeGender = gender) => {
    ...
    try {
      ...
      setError(err.message || "Failed to load carrom events");
    } finally {
      setLoading(false);
    }
  }, [gender]);
  ...
  const handleDelete = async (eventId: number) => {
    if (!confirm("Delete this carrom event from MongoDB?")) return;
    try {
      await deleteEvent(eventId, gender);
      await fetchEvents(gender);
      await fetchLeaderboard();
    } catch (err: any) {
      setError(err.message || "Failed to delete carrom event");
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || "Failed to save carrom event");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleUpdateEntry = async (t: CarromLeaderboardEntry) => {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black uppercase tracking-[0.18em] text-white">
              {gender === "men" ? "Men's Carrom" : "Women's Carrom"}
            </h1>
            <p className="mt-2 max-w-2xl text-sm text-zinc-400">
              Manual department-vs-department carrom event entry stored in MongoDB.
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 rounded-3xl border border-zinc-800 bg-zinc-950/80 p-6">
          <div className="flex flex-col gap-3 border-b border-zinc-800 pb-5 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-black uppercase tracking-wider text-white">
                {editingEventId !== null ? "Edit Carrom Event" : "Create Carrom Event"}
              </h2>
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <Field label="Event Name">
              <input value="carrom" readOnly className={readOnlyInputClass} />
            </Field>
            <Field label="Gender">
              <input value={gender} readOnly className={readOnlyInputClass} />
            </Field>
            <Field label="Event Status">
              <select value={formData.event_status} onChange={(e) => setFormData((prev) => ({ ...prev, event_status: e.target.value as ICarromEvent["event_status"] }))} className={inputClass}>
      ...
      <section className="overflow-hidden rounded-3xl border border-zinc-800 bg-zinc-950/80">
        <div className="flex flex-col gap-3 border-b border-zinc-800 px-6 py-5 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-black uppercase tracking-wider text-white">Carrom Events</h2>
          </div>
          <div className="text-sm text-zinc-400">
            Filtering MongoDB carrom events by <span className="font-black uppercase text-white">{gender}</span>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[920px] text-sm">
            ...
            <tbody className="divide-y divide-zinc-800">
              {loading ? (
                <tr><td colSpan={6} className="p-10 text-center text-[#FFBF00]">Loading carrom events...</td></tr>
              ) : events.length === 0 ? (
                <tr><td colSpan={6} className="p-10 text-center text-zinc-500">No carrom events found for this gender.</td></tr>
              ) : (
                events.map((event) => (
        ...
        {filteredEntries.length === 0 ? (
          ...
          <div className="divide-y divide-zinc-800/50">
            {groups.map(gp => {
              ...
              const gpTeams: CarromLeaderboardEntry[] = filteredEntries
                .filter(e => e.group === gp)
                .map(e => {
