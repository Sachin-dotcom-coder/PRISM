...
export default function LawnTennisAdminPage() {
  ...
  const handleEdit = (match: ILawnTennisMatch) => {
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-zinc-800/30">
                {loading && matches.length === 0 ? (
                  ...
                  <tr><td colSpan={7} className="p-10 text-center text-zinc-500">No {gender} tennis matches found. Create one.</td></tr>
                ) : (
                  matches.map((match) => (
                    <tr key={match.match_id} className="hover:bg-zinc-900/40 transition-colors">
                      ...
                      <td className="p-4 text-center font-mono text-zinc-300 font-bold bg-zinc-900/30">
                        {match.games?.map((g, idx) => (
                          <div key={idx} className="text-xs">
                            <span className="text-[#FFBF00]">{g.score_dept1}</span> - <span className="text-[#FFBF00]">{g.score_dept2}</span>
                          </div>
                        ))}
                      ...
                      <td className="p-4 text-center">
                        <span className={`px-2 py-1 text-[10px] uppercase font-[900] tracking-widest rounded border ${
                          ...
                          match.status === 'ongoing' ? 'border-[#FFBF00]/30 text-[#FFBF00] bg-[#FFBF00]/10 shadow-[0_0_8px_rgba(255,191,0,0.5)]' :
