...
import { createMatch, updateMatch } from '../services/handballApi';
import { DEPARTMENT_OPTIONS } from '../../shared/departmentOptions';

interface MatchFormProps {
  initialData?: IHandballMatch | null;
  gender: "men" | "women";
  onSuccess: () => void;
  onCancel: () => void;
}
...
export default function MatchForm({ initialData, gender, onSuccess, onCancel }: MatchFormProps) {
  const [formData, setFormData] = useState<IHandballMatch>({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (initialData?._id) {
         // Pass empty scores directly for deletion, else explicit numbers calculation is handled on the backend
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-zinc-900/50 p-6 rounded-2xl border border-zinc-800 space-y-6">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-[#FFBF00] font-bold text-lg">{initialData ? 'Edit Handball Match' : 'Create New Handball Match'}</h3>
      ...
      <div className="pt-4 border-t border-zinc-800">
        <h4 className="text-zinc-300 font-bold tracking-widest text-sm uppercase mb-4">Total Goals Scored</h4>
        
        <div className="grid grid-cols-2 gap-4">
           <div>
             <label className="block text-xs font-semibold text-[#FFBF00] uppercase mb-1">Team 1 Goals</label>
           ...
           <div>
             <label className="block text-xs font-semibold text-[#FFBF00] uppercase mb-1">Team 2 Goals</label>
