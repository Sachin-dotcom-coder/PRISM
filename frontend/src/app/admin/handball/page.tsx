...
import { getMatches, deleteMatch } from './services/handballApi';
import { useGender } from '@/app/components/Providers';

// ─── Types ────────────────────────────────────────────────────────────────────
interface HandballLeaderboardEntry {
  _id?: string;
...
const LB_API = `${BASE_URL}/handball-leaderboard`;
...
// ─── Leaderboard Row ──────────────────────────────────────────────────────────
function HandballTeamRow({
  team, rank, onUpdate, onDelete,
}: {
  team: HandballLeaderboardEntry;
  rank: number;
  onUpdate: (t: HandballLeaderboardEntry) => void;
  onDelete: () => void;
}) {
  ...
  return (
    <tr className="border-b border-[#FFBF00]/20 bg-[#FFBF00]/5">
      ...
      <td colSpan={3} className="p-2 text-center text-[10px] text-zinc-500 italic">Auto-calculated from completed matches</td>
      <td className="p-2">
        <div className="flex gap-1 justify-end">
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function HandballAdminPage() {
  ...
  // ── Match state ──
  const [matches, setMatches] = useState<IHandballMatch[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [showForm, setShowForm] = useState(false);
  const [editingMatch, setEditingMatch] = useState<IHandballMatch | null>(null);

  // ── Leaderboard state ──
  const [lbEntries, setLbEntries] = useState<HandballLeaderboardEntry[]>([]);
  const [standings, setStandings] = useState<Record<string, any[]>>({});
  const [addMode, setAddMode] = useState(false);
  const [lbMsg, setLbMsg] = useState('');
  const [newEntry, setNewEntry] = useState<Partial<HandballLeaderboardEntry>>({
    dept_name: '',
    category: lbCategory,
    group: 'A',
  });
  ...
  const fetchLeaderboard = useCallback(async () => {
    try {
      ...
      // getAllLeaderboardEntries returns { data: [] }
  ...
  const handleEdit = (match: IHandballMatch) => { setEditingMatch(match); setShowForm(true); };
  const handleDelete = async (match_id: number) => {
    if (!confirm('Are you sure you want to delete this match?')) return;
    try { await deleteMatch(match_id); fetchMatches(); fetchLeaderboard(); }
  ...
  const handleAddEntry = async () => {
    ...
    // Auto-generate leaderboard_id as max existing id + 1
  ...
  const handleUpdateEntry = async (t: HandballLeaderboardEntry) => {
    // handball backend uses leaderboard_id (numeric) as :id
  ...
  // Filter entries by current category
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-[900] tracking-widest text-[#FFBF00] uppercase">Handball CMS</h1>
          </div>
        </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-zinc-800/30">
                  {loading && matches.length === 0 ? (
                    <tr><td colSpan={7} className="p-10 text-center text-[#FFBF00] font-bold animate-pulse">Fetching Matches...</td></tr>
                  ) : matches.length === 0 ? (
                    <tr><td colSpan={7} className="p-10 text-center text-zinc-500">No {gender} matches found. Create one.</td></tr>
                  ) : (
                    matches.map((match) => (
                      <tr key={match.match_id} className="hover:bg-zinc-900/40 transition-colors">
                        ...
                        <td className="p-4">
                          <div className="text-white font-bold capitalize">{match.match_stage?.replace('_', ' ')}</div>
                          {(match as any).venue && <div className="text-zinc-500 text-xs capitalize">{(match as any).venue}</div>}
                        </td>
                        <td className="p-4">
                        ...
                        <td className="p-4 text-center font-mono text-zinc-300 font-black text-lg bg-zinc-900/30">
                          {typeof match.team1_score === 'number' && typeof match.team2_score === 'number' ? (
                            <>
                              <span className="text-[#FFBF00]">{match.team1_score}</span>
                              <span className="text-zinc-600 mx-1">-</span>
                              <span className="text-[#FFBF00]">{match.team2_score}</span>
                            </>
          ...
          <section className="bg-zinc-950/50 rounded-3xl border border-zinc-800 overflow-hidden backdrop-blur-xl">
            ...
            {filteredEntries.length === 0 ? (
              ...
              <div className="divide-y divide-zinc-800/50">
                {groups.map(gp => {
                  // standings are keyed by group and already computed by backend
                  // filter standings entries to only those matching this category from filteredEntries
                  const entryLookup: Record<string, HandballLeaderboardEntry> = {};
                  filteredEntries.filter(e => e.group === gp).forEach(e => { entryLookup[e.dept_name] = e; });

                  const rawStandings: any[] = standings[gp] ?? [];

                  // Show ALL registered teams; merge in standings data where available
                  const gpTeams: HandballLeaderboardEntry[] = filteredEntries
                    .filter(e => e.group === gp)
                    .map(e => {
