...
export default function HandballAdminPage() {
  ...
  const handleEdit = (match: IHandballMatch) => {
                ...
                {loading && matches.length === 0 ? (
                  ...
                  matches.map((match) => (
                    <tr key={match.match_id} className="hover:bg-zinc-900/40 transition-colors">
                      ...
                      <td className="p-4">
                        <div className="text-zinc-500 text-xs tracking-wider capitalize">{match.match_stage.replace('_', ' ')}</div>
                      </td>
                      <td className="p-4">
                      ...
                      <td className="p-4 text-center font-mono text-zinc-300 font-black text-lg bg-zinc-900/30">
                        {typeof match.team1_score === 'number' && typeof match.team2_score === 'number' ? (
                          <>
                             <span className="text-[#FFBF00]">{match.team1_score}</span> <span className="text-zinc-600">-</span> <span className="text-[#FFBF00]">{match.team2_score}</span>
                          </>
