...
const LB_API = `${BASE_URL}/volleylead`;
...
// ─── Leaderboard Row ──────────────────────────────────────────────────────────
function VolleyTeamRow({
  team, rank, onUpdate, onDelete,
}: {
  team: VolleyLeaderboardEntry;
  rank: number;
  onUpdate: (t: VolleyLeaderboardEntry) => void;
  onDelete: () => void;
}) {
...
export default function VolleyballAdminPage() {
  ...
  const [editingMatch, setEditingMatch] = useState<IVolleyballMatch | null>(null);

  // ── Leaderboard state ──
  const [lbEntries, setLbEntries] = useState<VolleyLeaderboardEntry[]>([]);
  ...
  const fetchMatches = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await getMatches(gender);
  ...
  const handleEdit = (m: IVolleyballMatch) => { setEditingMatch(m); setShowForm(true); };
  ...
  const handleUpdateEntry = async (t: VolleyLeaderboardEntry) => {
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-[900] tracking-widest text-[#FFBF00] uppercase">Volleyball CMS</h1>
          </div>
        </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-zinc-800/30">
                  {loading && matches.length === 0 ? (
                    ...
                    <tr><td colSpan={7} className="p-10 text-center text-zinc-500">No {gender} volleyball matches found. Create one.</td></tr>
                  ) : (
                    matches.map((m) => (
                      <tr key={m.match_id} className="hover:bg-zinc-900/40 transition-colors">
                        ...
                        <td className="p-4">
                          ...
                          <span className="font-[900] text-white tracking-widest">{m.team2_department}</span>
                        </td>
                        <td className="p-4 text-center bg-zinc-900/30">
                          {/* Per-set scores */}
                          {m.games && m.games.length > 0 ? (
                            <div className="space-y-0.5">
                              {m.games.map((g, i) => (
                                <div key={i} className="text-xs font-mono">
                                  <span className="text-zinc-400">S{g.game_number}: </span>
                                  <span className="text-[#FFBF00] font-black">{g.team1_score}</span>
                                  <span className="text-zinc-500 mx-1">-</span>
                                  <span className="text-[#FFBF00] font-black">{g.team2_score}</span>
                                </div>
                              ))}
                              {/* Set summary */}
                              <div className="mt-1 text-[11px] font-black text-white">
                                {m.team1_score ?? 0} – {m.team2_score ?? 0} sets
                              </div>
                            </div>
                          ) : (
                        ...
                        <td className="p-4 text-center font-[900] text-white tracking-widest">{m.winner || '-'}</td>
                        <td className="p-4 text-right">
                          <div className="flex gap-2 justify-end">
                            <button onClick={() => handleEdit(m)} className="p-2 rounded bg-zinc-800 hover:bg-[#FFBF00] transition-all text-zinc-400 hover:text-black shadow-md"><Pencil className="w-4 h-4"/></button>
                            <button onClick={() => handleDelete(m.match_id)} className="p-2 rounded bg-zinc-800 hover:bg-red-600 transition-all text-zinc-400 hover:text-white shadow-md"><Trash2 className="w-4 h-4"/></button>
                          </div>
                        </td>
                      </tr>
                    ))
            ...
            {filteredEntries.length === 0 ? (
              ...
              <div className="divide-y divide-zinc-800/50">
                {groups.map(gp => {
                  ...
                  const entryLookup: Record<string, VolleyLeaderboardEntry> = {};
                  filteredEntries.filter(e => e.group === gp).forEach(e => { entryLookup[e.dept_name] = e; });

                  const gpTeams: VolleyLeaderboardEntry[] = rawStandings.length > 0
                    ? rawStandings
