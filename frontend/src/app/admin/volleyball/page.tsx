...
export default function VolleyballAdminPage() {
  ...
  const handleEdit = (match: IVolleyballMatch) => {
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-zinc-800/30">
                {loading && matches.length === 0 ? (
                  <tr><td colSpan={7} className="p-10 text-center text-[#FFBF00] font-bold animate-pulse">Fetching {gender}'s Matches...</td></tr>
                ) : matches.length === 0 ? (
                  <tr><td colSpan={7} className="p-10 text-center text-zinc-500">No {gender}'s volleyball matches found. Create one.</td></tr>
                ) : (
                  matches.map((match) => (
                    <tr key={match.match_id} className="hover:bg-zinc-900/40 transition-colors">
                      ...
                      <td className="p-4">
                        <div className="text-white font-bold capitalize">{match.match_stage}</div>
                      </td>
                      <td className="p-4">
                      ...
                      <td className="p-4 text-center font-mono text-zinc-300 font-bold bg-zinc-900/30">
                        {match.team1_score !== undefined && match.team2_score !== undefined && match.team1_score !== '' && match.team2_score !== '' ? (
                          <div className="flex items-center justify-center text-sm">
                            <span className={Number(match.team1_score) > Number(match.team2_score) ? "text-[#FFBF00]" : "text-zinc-500"}>{match.team1_score}</span>
                            <span className="mx-2 text-zinc-600">-</span>
                            <span className={Number(match.team2_score) > Number(match.team1_score) ? "text-[#FFBF00]" : "text-zinc-500"}>{match.team2_score}</span>
                          </div>
                        ) : (
