...
export default function BasketballAdminPage() {
  ...
  const handleEdit = (match: IBasketballMatch) => {
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-zinc-800/30">
                {loading && matches.length === 0 ? (
                  ...
                  <tr><td colSpan={7} className="p-10 text-center text-zinc-500">No {gender}'s basketball matches found. Create one.</td></tr>
                ) : (
                  matches.map((match) => (
                    <tr key={match.match_id} className="hover:bg-zinc-900/40 transition-colors">
                      ...
                      <td className="p-4">
                        ...
                        {match.venue && <div className="text-zinc-500 text-xs tracking-wider capitalize">{match.venue}</div>}
                      </td>
                      <td className="p-4">
                      ...
                      <td className="p-4 text-center font-mono text-zinc-300 font-bold bg-zinc-900/30">
                        {match.games && match.games.length > 0 ? (
                          <div className="text-lg">
                            <span className="text-[#FFBF00]">{match.games[0].team1_score}</span>
                            <span className="mx-2 text-zinc-500">-</span>
                            <span className="text-[#FFBF00]">{match.games[0].team2_score}</span>
                          </div>
                        ) : (
